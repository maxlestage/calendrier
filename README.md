...
- **Événements thématiques générés à chaque démarrage**, pour l'année en cours **et** la suivante :
  ...
  - 🌊 **Marées** (bleu mer) : pleines et basses mers avec heures et hauteurs, via l'[API WorldTides](https://www.worldtides.info) (prédictions officielles SHOM/FES). Catalogue par **groupes de côtes**, utilisables tels quels dans `TIDE_PORTS` :
    - `ocean` (défaut) : Biarritz, Hossegor, Lacanau-Océan, Arcachon, Les Sables-d'Olonne, La Baule
    - `mer` (défaut) : Marseille, Palavas-les-Flots, Cap d'Agde, Argelès-sur-Mer, Cannes, Nice, Ajaccio — marnage méditerranéen faible (~20-40 cm), le vent/la pression comptent souvent plus que la marée
    - `manche` : Saint-Malo, Deauville, Le Touquet, Étretat
    - `ports` : Brest, La Rochelle
    `TIDE_PORTS` mélange groupes et spots (ex. `ocean,nice` ou `biarritz,lacanau`) — moins de spots = agenda plus lisible (4 marées/jour/spot) et moins de quota. **Activation par clé** : config var `WORLDTIDES_API_KEY` — sans clé, pas de marées. Un modèle « maison » a été écarté à dessein : des horaires de marée faux sont dangereux (baïnes, pêche à pied, estran). `TIDE_DAYS` : horizon en jours (défaut 14) ; l'appel API n'est fait que lorsque l'horizon stocké d'un spot devient court
