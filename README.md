...
- **Événements thématiques générés à chaque démarrage**, pour l'année en cours **et** la suivante :
  ...
  - 🌊 **Marées** (bleu mer) : pleines et basses mers avec heures et hauteurs pour **Brest, Saint-Malo, La Rochelle et Nice**, via l'[API WorldTides](https://www.worldtides.info) (prédictions officielles SHOM/FES). **Activation par clé** : ajoutez la config var `WORLDTIDES_API_KEY` (clé sur worldtides.info) — sans clé, pas de marées. Un modèle « maison » a été écarté à dessein : des horaires de marée faux sont dangereux (pêche à pied, estran). Réglable : `TIDE_PORTS` (liste de ports, ex. `brest,saint-malo`), `TIDE_DAYS` (horizon en jours, défaut 14). L'appel API n'est fait que lorsque l'horizon stocké d'un port devient court, pour économiser le quota
