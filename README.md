...
- **Événements thématiques générés à chaque démarrage**, pour l'année en cours **et** la suivante :
  ...
  - 🌊 **Marées** (bleu mer) : pleines et basses mers avec heures et hauteurs, via l'[API WorldTides](https://www.worldtides.info) (prédictions officielles SHOM/FES). Par défaut, les **plages océanes** : Biarritz, Hossegor, Lacanau-Océan, Arcachon, Les Sables-d'Olonne, La Baule. Aussi au catalogue (via `TIDE_PORTS`) : Brest, Saint-Malo, La Rochelle, Nice. **Activation par clé** : config var `WORLDTIDES_API_KEY` (clé sur worldtides.info) — sans clé, pas de marées. Un modèle « maison » a été écarté à dessein : des horaires de marée faux sont dangereux (pêche à pied, baïnes, estran). Réglable : `TIDE_PORTS` (ex. `biarritz,lacanau` — moins de spots = moins d'événements par jour et moins de quota), `TIDE_DAYS` (horizon en jours, défaut 14). L'appel API n'est fait que lorsque l'horizon stocké d'un spot devient court
